...
The provider is not selected by this document. For the current ChatGPT Pro seat, [OpenAI documents custom read/fetch MCP apps in developer mode on the web](https://help.openai.com/en/articles/12584461-developer-mode-and-mcp-apps-in-chatgpt). The six read-only tools must pass ChatGPT's actual tool scan before availability is claimed.
