...
The application process itself does not remain root after startup.
...
3. Protected Resource Metadata is reachable for both `/mcp` and `/chatgpt/mcp`.
4. Unauthenticated MCP calls return an OAuth Bearer challenge rather than executing.
5. OAuth can issue an audience-bound token for `/chatgpt/mcp`.
6. A real device pairs and registers the native catalog.
7. `list_devices` from `/chatgpt/mcp` sees that device.
8. A remote read succeeds.
9. A remote write/process call enters the expected approval/policy path and then succeeds only after permitted confirmation.
10. The agent is deliberately terminated and the supervisor brings it back online.
11. A device revoke/re-pair recovery drill succeeds.
12. A rollback to the previous known-good service revision is tested.
